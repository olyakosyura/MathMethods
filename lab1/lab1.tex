...
\author{Косюра Ольга Владиславовна \\ Группа ИУ7-27 \\ Вариант \textnumero 8}
...
\begin{enumerate}
	...
	\item{Удельная теплоёмкость стержня \(c = 2\)}
	\item{Начальная температура стержня \(u_0 = 0.2\)}
	\item{Постоянные: \(a = 0.5\), \(b = 2\), \(\sigma = 1.25\)}
	\item{Коэфициент теплопроводности \(K(u) = a + bu^\sigma = 0.5 + 2*u^{1.25}\)}
	\item{Начальное распределение температуры \(\varphi (x) \equiv u_0\)}
	\item{Тепловой поток на правом конце стержня}
\begin{equation}
	W_4(t) = \begin{cases}
							2Qt, &\text{$0 \leqslant  t < 0.5*t_0$}\\
							2Q(t_0-t), &\text{$0.5*t_0 \leqslant  t < t_0$}\\
							0, &\text{$t_0 \leqslant t$}
				 			\end{cases}
\end{equation}
	\item{\(t_0 = 0.5\), \(Q = 10\)}
	\item{Тепловой поток на левом конце стержня равен 0.}
\end{enumerate}
...
\left.K(u)\frac{\partial u}{\partial x}\right |_{x=l} = W_4(t)
\end{array} 
\right.
\end{displaymath}
...
Граничные условия:
\begin{equation}
K_{0+}^{j+1}\frac{u_1^{j+1} - u_0^{j+1}}{h} = W|_{x=0} = 0;  \qquad
K_{N-}^{j+1}\frac{u_N^{j+1} - u_{N-1}^{j+1}}{h} = W|_{x=l} = W_4(t);
\end{equation}

Следовательно:
\begin{equation}
u_1^{j+1} - u_0^{j+1} = 0;   \qquad
u_N^{j+1} - u_{N-1}^{j+1} = \frac{W_4h}{K_{0+}^{j+1}};
\end{equation}
...
\begin{figure}[h]
\centering
\includegraphics[width = 16cm]{screen.jpg}
